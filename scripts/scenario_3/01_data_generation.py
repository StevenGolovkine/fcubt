...
def simulation(i):
	...
	with open(f'./data/scenario_3_{i}_smooth.pkl', 'wb') as f:
	...
	np.savetxt(f'./data/scenario_3_{i}_A_smooth.csv', data_1_smooth.values,delimiter=',')
	np.savetxt(f'./data/scenario_3_{i}_B_smooth.csv', data_2_smooth.values,delimiter=',')
	np.savetxt('./data/labels.csv', labels, delimiter=',')

if __name__ == "__main__":
	results = Parallel(n_jobs=num_cores)(delayed(simulation)(i) 
		for i in np.arange(415, 500, 1))
